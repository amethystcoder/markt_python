...
class PendingOrders(MethodView):
    ...
    def get(self, seller_id):
        try:
            return [parse_unaccepted_orders(order=order, buyer=Buyer(order.buyer_id), product=Product(order.product_id),
                                            product_image=ImageNameStore.getproductthumbnail(order.product_id)) for
                    order in Order.get_seller_pending_orders(seller_id)]
        except Exception as e:
            abort(500, "could not create")
